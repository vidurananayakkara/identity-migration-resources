...
ALTER TABLE IDN_OAUTH2_ACCESS_TOKEN DROP CONSTRAINT CON_APP_KEY CASCADE
/
...
BEGIN
    ...
    WHILE row_offset < oidc_scope_count LOOP
        SELECT ID INTO oidc_scope_id
        FROM (
            SELECT ROWNUM OFFSET, ID
            FROM IDN_OIDC_SCOPE
            )
        WHERE OFFSET > row_offset AND ROWNUM <= 1;

        INSERT INTO IDN_OAUTH2_SCOPE (NAME, DISPLAY_NAME, TENANT_ID, SCOPE_TYPE)
        SELECT n1, n2, TENANT_ID, 'OIDC'
        FROM (
            SELECT ROWNUM OFFSET, NAME n1, NAME n2, TENANT_ID, 'OIDC'
            FROM IDN_OIDC_SCOPE
            )
        WHERE OFFSET > row_offset AND ROWNUM <= 1;
